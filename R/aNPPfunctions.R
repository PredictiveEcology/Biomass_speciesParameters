...
modifySpeciesTable <- function(gamms, speciesTable, factorialTraits, factorialBiomass, sppEquiv,
  ...
  best <- newTraits[, .(growthcurve = round(sum(AICWeightsStd * growthcurve), 2),
                        ...
                        inflationFactor = round(sum(AICWeightsStd * inflationFactor), 3)), by = "species"]
  ...
  newTraits <- updateInflationFactor(newTraits, factorialTraits, factorialBiomass)
...
makeGAMMdata <- function(species, psp, speciesEquiv,
  ...
  simData <- rbindlist(list(spDominant, simulatedData), fill = TRUE)
  ...
  # plot(simData2$standAge, simData2$biomass, pch = 19, cex = 0.5, col = ma, xlim = c(0, 150), ylim = c(0, 30000))
  ...
  if (FALSE) {
    plot(simData2$standAge, simData2$biomass, pch = 19, cex = 0.5)
    points(simData2$standAge, fitted(nlsouts$CR), pch = 19, col = "green", cex = 0.5)
    points(simData2$standAge, fitted(nlsouts$Gompertz), pch = 19, col = "blue", cex = 0.5)
    points(simData2$standAge, fitted(nlsouts$Logistic), pch = 19, col = "red", cex = 0.5)
    legend(col = c("green", "blue", "red"),
           legend = paste(names(nlsouts), "AIC: ", sapply(nlsouts, function(x) round(AIC(x), 1 ))),
           "topleft", pch = 19)
  }
  nlsout <- nlsouts[[which.min(sapply(nlsouts, function(x) if (is(x, "try-error")) 1e12 else AIC(x)))]]
...
editSpeciesTraits <- function(name, gamm, traits, fT, fB, speciesEquiv, sppCol,
  ...
  nlsoutput <- tryCatch(predict(Gamm$NonLinearModel, predData), error = function(x) 0)
  if (!is(output, "list")) {
    o <- output
    output <- list()
    output$fit <- o
  ...
  predData <- data.table("age" = predData$standAge, "predBiomass" = output$fit, 
                         "predSE" = output$se.fit, "predBiomassNonLin" = nlsoutput)
  ...
  if (TRUE) { # This is about plotting (Eliot Nov 18, 2021) -- couldn't fit into the module in a modular way 
  ...
  if (NROW(bestTraits) > 1) {
    traits <- rbindlist(lapply(seq(NROW(bestTraits)), function(x) traits))
  } 
