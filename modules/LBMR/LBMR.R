...
defineModule(sim, list(
  ...
  parameters = rbind(
    ...
    defineParameter('gmcsRatioLimits', 'numeric', c(1/1.5, 1.5/1), NA, NA, desc = "if using LandR.CS for climate-sensitive growth and mortality,
                    a ratio is used to estimate the effect of climate on growth/mortality (currentClimate/referenceClimate). Upper and lower limits are suggested 
                    to circumvent problems caused by very small denominators as well as predictions outside the data range used to generate the model"),
...
doEvent.LBMR <- function(sim, eventTime, eventType, debug = FALSE) {
  ...
  switch(eventType,
         init = {
           ...
           sim <- scheduleEvent(sim, start(sim), "LBMR", "save2", eventPriority =  9)
         },
         summaryBGMstart = {
           sim <- SummaryBGM(sim)
         ...
         save2 = {
           ...
           spec <- species[, .(species, longevity, growthcurve, mortalityshape, mANPPproportion)]
           cohortData[, c("climGrowth", "climMort", 'ecoregionGroup', 'pixelGroup') := NULL]
           cohortData <- cohortData[spec, on = c('speciesCode' = 'species')]
           
           saveRDS(cohortData, file = paste0(outputPath(sim), "/cohortData", time(sim)))
           sim <- scheduleEvent(sim, time(sim) + 1, "LBMR", "save2", eventPriority = 9)
         },
...
MortalityAndGrowth <- function(sim) {
  ...
  for (subgroup in groupNames) {
    ...
    subCohortData[, age := age + 1L] #This was preventing my cohorts from aging
...
Save <- function(sim) {
  # saveRDS(sim$cohortData, file = file.path(outputPath(sim), paste0("cohortData", time(sim))))
