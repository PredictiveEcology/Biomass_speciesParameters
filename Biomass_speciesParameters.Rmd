...
traits that allows a better match to the observed curves.
...
`mANPPproportion` that are used to calibrate previously estimated species
...
SK, AB, and BC. The processed and anonymized PSP data is however provided via a
Google Drive folder that is accessed automatically by the module.
...
resulting dynamics will be different under in a multi-species context.
...
attributes anonymized. We only share the anonymized dataset, as the data sharing
agreements must be met to utilise the raw data.
...
species trait combination in the study area landscape. The library of curves are
created by running *Biomass_core* with no reproduction, competition,
disturbance, or dispersal effects, on the study area. This cohort data table is
the table coming out of *Biomass_borealDataPrep* with each simulation varied in
...
The values for `maxB` (the calibrated parameter) and `maxANPP` were explored via
the `mANPPproportion`, the ratio of `maxANPP` to `maxBiomass` (the parameter
used for theroetical curves), as it reflects their relationship.
...
The module attempts to use stands where the focal species is dominant (but not
monocultures) to estimate parameters, while balancing sample size (see biomass
weighting below). Hence, for a given species, it only includes plots where the
...
\@ref(tab:moduleInputs2-Biomass-speciesParameters)). The only inputs that
...
take place. This input object **must be supplied by the user**.
...
geographical data of the PSP datasets used to buildi observed species growth
curves.
...
ID). 'maxB' and 'maxANPP' values will be calibrated by species.

```{r moduleInputs2-Biomass-speciesParameters, cache.rebuild = TRUE, echo = FALSE, eval = TRUE, message = FALSE}
...
-   `GAMMiterations` and `GAMMknots` -- control the number of iterattions and
